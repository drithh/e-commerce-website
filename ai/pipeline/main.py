...
class ImageClassifier:
    def __init__(self):
        ...
        self.classifiers.load_state_dict(torch.load(model_path, map_location=torch.device('cpu')))
    ...
    def augmentation(self,image):
      list_image = []
      list_image.extend((cv2.rotate(image, cv2.ROTATE_180), cv2.flip(image, 1), cv2.flip(image, 0), cv2.flip(image, -1), self.threshold(image), self.edges(image)))
      return list_image
